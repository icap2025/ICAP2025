...
const InvitedTalk = [
  ...
  {
    name: "Professor Dr. Belal Hossen",
    university: "Department of Physics, CUET",
    image: "/guest/Professor Dr. Belal Hossen.jpeg",
    bio: `Dr. Mohammad Belal Hossen is a distinguished physicist and Professor at the Department of Physics, Chittagong University of Engineering & Technology (CUET), Bangladesh. His research focuses on functional nanomaterials, ferrites, multiferroics, and the dielectric and electromagnetic properties of advanced ceramic materials. With a strong publication record and significant citation impact, his work has contributed notably to the understanding and application of multifunctional materials.`,
    status: "Invited Speaker",
  },
  {
    name: "Professor Dr. Syed Badiuzzaman Faruque",
    university: "Department of Physics, SUST",
    image: "/guest/Professor Dr. Syed Badiuzzaman Faruque.png",
    bio: `Prof. S. B. Faruque is a Professor of Physics at Shahjalal University of Science and Technology (SUST), Sylhet, with research expertise spanning quantum mechanics, general relativity, and celestial physics. His scholarly work reflects significant contributions to theoretical and applied physics.`,
    status: "Invited Speaker",
  },
];
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative mb-4 sm:mb-6 md:mb-8">
        <div className="sm:border-6 xs:h-32 xs:w-32 lg:border-6 relative h-28 w-28 overflow-hidden rounded-full border-4 border-primary shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-2xl sm:h-40 sm:w-40 md:h-48 md:w-48 lg:h-56 lg:w-56">
          <Image
            src={speaker.image}
            alt={speaker.name}
            fill
        ...
        <div className="xs:-bottom-3 xs:px-4 xs:py-1.5 xs:text-xs absolute -bottom-2 left-1/2 -translate-x-1/2 transform whitespace-nowrap rounded-full bg-primary px-3 py-1 text-[10px] font-bold uppercase text-white shadow-lg sm:-bottom-4 sm:px-6 sm:py-1.5 sm:text-sm">
          {speaker.status}
        </div>
      </div>

      <h3 className="xs:text-base mt-2 px-2 text-center text-sm font-bold text-gray-900 sm:mt-4 sm:text-lg md:text-xl lg:text-2xl">
        {speaker.name}
      </h3>
      <p className="xs:text-xs mt-1 px-2 text-center text-[11px] leading-relaxed text-gray-700 sm:mt-2 sm:text-sm md:text-base">
        {speaker.university}
      </p>
      ...
      <div className="mt-4 w-full px-2 text-center sm:mt-6 sm:px-4 md:mt-8">
        <div className="relative rounded-xl bg-gradient-to-br from-cyan-50/80 via-blue-50/80 to-teal-50/80 p-2 shadow-sm sm:p-4">
          <span className="xs:-left-2 xs:-top-3 xs:text-3xl absolute -left-1 -top-2 font-serif text-2xl text-primary opacity-30 sm:-left-3 sm:-top-4 sm:text-4xl md:text-5xl">
            &quot;
          </span>
          <p className="xs:px-3 xs:py-2 xs:text-xs px-2 py-1 text-[11px] italic leading-relaxed text-gray-800 sm:px-6 sm:text-sm md:text-base">
            {speaker.bio}
          </p>
          <span className="xs:-bottom-3 xs:-right-2 xs:text-3xl absolute -bottom-2 -right-1 font-serif text-2xl text-primary opacity-30 sm:-bottom-4 sm:-right-3 sm:text-4xl md:text-5xl">
            &quot;
          </span>
        </div>
      </div>
  ...
  return (
    <div className="w-full bg-white py-6 sm:py-8 md:py-12 lg:py-16 xl:py-20">
      <div className="xs:px-4 mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
        {/* Title */}
        <div className="xs:text-2xl mb-3 text-center text-xl font-bold uppercase tracking-tight text-gray-900 sm:mb-4 sm:text-3xl md:mb-6 md:text-4xl lg:mb-8 lg:text-5xl">
          Distinguished Invited Speakers
          <hr className="mx-auto mt-1 w-4/5 border-2 border-primary shadow-xl sm:mt-2 md:mt-3 lg:w-3/5" />
        </div>

        {/* Description */}
        <p className="xs:text-sm mx-auto mb-6 max-w-4xl text-center text-xs font-light leading-relaxed text-gray-700 sm:mb-8 sm:text-base md:mb-10 md:text-lg lg:mb-12 lg:text-xl">
        ...
        <div className="mt-6 md:hidden">
          <div className="relative">
            ...
            <div className="xs:min-h-[520px] min-h-[360px] sm:min-h-[500px]">
            ...
            <button
              onClick={prevSlide}
              className="xs:p-2 absolute left-0 top-20 -translate-y-1/2 rounded-full bg-primary p-1.5 text-white shadow-lg transition-all duration-300 hover:scale-110 hover:bg-primary/90 active:scale-95 sm:p-3"
              aria-label="Previous speaker"
            >
              <ChevronLeft className="xs:h-5 xs:w-5 h-4 w-4 sm:h-6 sm:w-6" />
            </button>

            <button
              onClick={nextSlide}
              className="xs:p-2 absolute right-0 top-20 -translate-y-1/2 rounded-full bg-primary p-1.5 text-white shadow-lg transition-all duration-300 hover:scale-110 hover:bg-primary/90 active:scale-95 sm:p-3"
              aria-label="Next speaker"
            >
              <ChevronRight className="xs:h-5 xs:w-5 h-4 w-4 sm:h-6 sm:w-6" />
            </button>
          </div>
          ...
          <div className="flex justify-center gap-2">
            {InvitedTalk.map((_, index) => (
              <button
                ...
                className={`xs:h-2.5 h-2 rounded-full transition-all duration-300 ${
                  index === currentIndex
                    ? "xs:w-8 w-6 bg-primary sm:w-10"
                    : "xs:w-2.5 w-2 bg-gray-300 hover:bg-gray-400"
                }`}
